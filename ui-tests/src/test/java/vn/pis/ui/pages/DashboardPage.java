...
public class DashboardPage {
    ...
    private final By ALERT_ITEMS = By.xpath(
        "//main//div[contains(@class,'p-6') and contains(@class,'space-y-4')]//div[contains(@class,'bg-white') and contains(@class,'border-l-8') and contains(@class,'shadow-sm')]"
    );
    ...
    private final By SIDEBAR = By.xpath(
        "//div[contains(@class,'w-64') and contains(@class,'bg-card') and contains(@class,'border-r')]"
    );

    private final By SIDEBAR_MENU_ITEMS = By.xpath(
        "//div[contains(@class,'w-64') and contains(@class,'bg-card')]//nav//a"
    );

    // =========================================================
    // LOCATORS - NOTIFICATION / USER
    // =========================================================
    private final By NOTIFICATION_BELL_BTN = By.xpath("//button[./svg[contains(@class,'lucide-bell')]]");
    private final By NOTIFICATION_BADGE = By.xpath("//button[.//*[contains(@class,'lucide-bell')]]//div[contains(@class,'rounded-full')]");
    ...
    public boolean isKpiTotalTypesVisible() {
        try { waitVisible(KPI_LABEL_TOTAL_TYPES); waitVisible(KPI_VALUE_TOTAL_TYPES); return true; }
        catch(Exception e){ return false; }
    }

    public boolean isKpiTotalValueVisible() {
        try { waitVisible(KPI_LABEL_TOTAL_VALUE); waitVisible(KPI_VALUE_TOTAL_VALUE); return true; }
        catch(Exception e){ return false; }
    }

    public boolean isKpiExpiringVisible() {
        try { waitVisible(KPI_LABEL_EXPIRING); waitVisible(KPI_VALUE_EXPIRING); return true; }
        catch(Exception e){ return false; }
    }

    public boolean isKpiBelowMinVisible() {
        try { waitVisible(KPI_LABEL_BELOW_MIN); waitVisible(KPI_VALUE_BELOW_MIN); return true; }
        catch(Exception e){ return false; }
    }
    ...
    private boolean hasColorClassOnSelfOrParent(WebElement el) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            try { cur = cur.findElement(By.xpath("..")); }
    ...
    public int getImportBarsCount() { waitChartMounted(); return els(CHART_BARS_IMPORT).size(); }
    public int getExportBarsCount() { waitChartMounted(); return els(CHART_BARS_EXPORT).size(); }
    ...
    private boolean tooltipVisibleNow() {
        ...
        for (WebElement w : wrappers) {
            try {
                String vis = w.getCssValue("visibility");
    ...
    public boolean isChartTooltipVisible() {
        try { return wait.until(d -> tooltipVisibleNow()); }
    ...
    public boolean isUserAvatarVisible() {
        try { return waitVisible(USER_AVATAR).isDisplayed(); }
        catch (Exception e) {
