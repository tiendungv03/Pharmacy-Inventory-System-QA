...
public class MedicineListPage {
    ...
    public final By pageSubtitle = By.xpath("//p[contains(@class,'text-muted-foreground') and contains(normalize-space(.),'Quản lý toàn bộ thuốc trong kho')]");
    ...
    public final By categoryDropdown = By.xpath("//label[normalize-space()='Danh mục thuốc']/following-sibling::button");
    ...
    public final By emptyMessageCell = By.xpath("//td[@colspan='7' and contains(@class,'text-center') and normalize-space()='Không có sản phẩm nào']");
    ...
    public final By rowsPerPageButton = By.xpath("//span[normalize-space()='Số dòng/trang:']/following-sibling::button");
    public final By rowsPerPageValue  = By.xpath("//span[normalize-space()='Số dòng/trang:']/following-sibling::button/span[@style='pointer-events: none;']");
    ...
    public Range parseRange(String rangeText) {
        ...
        if (parts.length < 2) return new Range(0, 0);
    ...
    public String pickKeywordFromName(String name) {
        ...
        if (cleanedAll.length() >= 3) return cleanedAll.substring(0, 3);
        return norm(name);
    }
    ...
    public void waitForSearchAppliedStable(String keyword) {
        ...
        wait.until(d -> {
            ...
            for (int i = 0; i < sample; i++) {
                ...
                if (!(code.contains(kw) || name.contains(kw))) return false;
            }
            return true;
        });
    ...
    public void closeDropdownByEscape() {
        ...
        wait.until(d -> {
            ...
            for (WebElement o : opts) {
                try { if (o.isDisplayed()) return false; } catch (Exception ignored) {}
            }
            return true;
        });
    }
    ...
    public String selectFirstNonAllOption(By dropdownTrigger) {
        ...
        for (WebElement o : opts) {
            ...
            if (t.equalsIgnoreCase("Tất cả")) continue;
            chosen = t;
            break;
        }
        if (chosen == null) Assert.fail("Không tìm được option nào khác 'Tất cả'");
    ...
    public void waitForCategoryFilterApplied(String chosenCategory) {
        wait.until(d -> {
            ...
            for (WebElement r : rows) {
                ...
                checked++;
                if (!chosenCategory.equals(cat)) return false;
            }
            return checked > 0;
        });
        waitForTableStable(TABLE_STABLE_MS, TABLE_TIMEOUT_MS);
    ...
    public void waitForSupplierFilterApplied(String chosenSupplier) {
        wait.until(d -> {
            ...
            for (WebElement r : rows) {
                ...
                checked++;
                if (!chosenSupplier.equals(supp)) return false;
            }
            return checked > 0;
        });
        waitForTableStable(TABLE_STABLE_MS, TABLE_TIMEOUT_MS);
    ...
    public void waitForCategoryAndSupplierApplied(String chosenCategory, String chosenSupplier) {
        wait.until(d -> {
            ...
            for (WebElement r : rows) {
                String cat = getCellText(r, COL_CATEGORY);
                String supp = getCellText(r, COL_SUPPLIER);
                if (cat.isEmpty() || supp.isEmpty()) continue;
                checked++;
                if (!chosenCategory.equals(cat)) return false;
                if (!chosenSupplier.equals(supp)) return false;
            }
    ...
    public void changeRowsPerPage(String value) {
        ...
        try { wait.until(ExpectedConditions.elementToBeClickable(option)).click(); }
        catch (Exception e) { jsClick(option); }

        wait.until(ExpectedConditions.textToBe(rowsPerPageValue, value));
        try { waitForRangeChange(oldRange); } catch (Exception ignored) {}
    ...
    public void ensureRowsPerPage(String value) {
        String cur = "";
        try { cur = norm(driver.findElement(rowsPerPageValue).getText()); } catch (Exception ignored) {}
        if (!value.equals(cur)) changeRowsPerPage(value);
        else waitForTableStable(TABLE_STABLE_MS, TABLE_TIMEOUT_MS);
    }
    ...
    public void clickNextReliable() {
        ...
        try { next.click(); } catch (Exception e) { jsClick(next); }

        waitForRangeChange(before);
        if (oldFirst != null) {
            try { wait.until(ExpectedConditions.stalenessOf(oldFirst)); } catch (Exception ignored) {}
        }
        waitForTableStable(TABLE_STABLE_MS, TABLE_TIMEOUT_MS);
    }
    ...
    public void clickPrevReliable() {
        ...
        try { prev.click(); } catch (Exception e) { jsClick(prev); }
    ...
    public boolean isTrulyDisabled(WebElement el) {
        ...
        try { if (!el.isDisplayed()) return true; } catch (Exception e) { return true; }

        String disabled = el.getAttribute("disabled");
        String aria = el.getAttribute("aria-disabled");
        String cls = el.getAttribute("class");
        String pe = "";
        try { pe = el.getCssValue("pointer-events"); } catch (Exception ignored) {}

        if (disabled != null) return true;
        if ("true".equalsIgnoreCase(aria)) return true;
        if (cls != null && cls.toLowerCase().contains("disabled")) return true;
        if ("none".equalsIgnoreCase(pe)) return true;
    ...
    public String getAccessibleName(WebElement el) {
        ...
        if (!norm(ariaLabel).isEmpty()) return norm(ariaLabel);
        ...
        if (!norm(ph).isEmpty()) return norm(ph);
    ...
    public void openDropdownByKeyboard(By dropdownTrigger) {
        ...
        if (driver.findElements(dropdownOptions).isEmpty()) sendKeyToActive(Keys.SPACE);
    ...
    public void assertExpiryFormatOrDash(String expiry) {
        if ("-".equals(expiry)) return;

        Assert.assertTrue(expiry.matches("^\\d{2}/\\d{2}/\\d{4}$"),
                "Hạn sử dụng sai format (dd/MM/yyyy): " + expiry);

        DateTimeFormatter fmt = DateTimeFormatter.ofPattern("dd/MM/uuuu");
        try { LocalDate.parse(expiry, fmt); }
        catch (Exception e) { Assert.fail("Hạn sử dụng không phải ngày hợp lệ: " + expiry); }
    }
